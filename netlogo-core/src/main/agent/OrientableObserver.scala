...
import org.nlogo.api,
  api.{ AgentException, AgentFollowingPerspective, ObserverOrientation, Perspective, Vect }
...
trait OrientatableObserver { this: Observer =>
  ...
  class Orientation(observer: Observer) extends ObserverOrientation {
    ...
    def dx: Double = {
      ...
      if (StrictMath.abs(value) < org.nlogo.api.Numbers.Infinitesimal) 0
      else value;
    }
    ...
    def dz: Double = {
      val value = StrictMath.sin(StrictMath.toRadians(_orientation.pitch))
      if (StrictMath.abs(value) < org.nlogo.api.Numbers.Infinitesimal) 0
      else value
    }
