  ...
  def copyAgents(other: AgentManagement, newWorld: World): Unit = {
    ...
    breeds.asScala.foreach {
      case (name, agentset) if turtleBreedShapes.breedHasShape(agentset) =>
        newWorld.turtleBreedShapes.setBreedShape(agentset, turtleBreedShapes.breedShape(agentset))
      case _ =>
    }
    newWorld.linkShapes.addAll(linkShapes.shapeList.shapes)
    linkBreeds.asScala.foreach {
      case (name, agentset) if linkBreedShapes.breedHasShape(agentset) =>
        newWorld.linkBreedShapes.setBreedShape(agentset, linkBreedShapes.breedShape(agentset))
      case _ =>
    }
